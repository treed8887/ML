...
rm(agegroup,vaxstatus,total,severe) # oncethe vectors are in a tibble detele them
delta
```

Claim:  Almost 60% of those hospitalized for Covid-19 in Isreal as of August 15, 2021 were vaccinated.
...
ggplot(byage,aes(vaxstatus,severeper100k)) +
  ...
  theme_bw()+
  geom_text(aes(label = round(severeper100k,1)), vjust = -.2) +
  facet_grid(cols = vars(vaxstatus))
```
